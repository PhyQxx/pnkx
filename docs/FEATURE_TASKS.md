...
- [x] B5 **年度报告**（2026-09-23：生活报告页新增「今年」维度——后端 buildReportData 支持 period=year（当年 1.1 起）并增强聚合：支出分类 Top5（全周期通用）+ 12 个月支出分布（年报专属）；AI 年报专属 prompt（年度关键词/消费盘点/高光时刻/写给明年）；前端月度支出柱状图（纯 CSS 零依赖）+ 分类 Top5 排行（金银铜徽标）+ 历史报告三态标签；顺手修复待办统计未按周期过滤的旧 bug）
- [x] B6 登录设备管理（2026-09-23 核实：RuoYi 体系自带——`/monitor/online` 在线用户列表 + forceLogout 强退接口 + pnkx-ui 对应页面均已存在，评估时误报缺失；2FA 个人博客暂无必要，不做）
...
- 2026-09-23：完成 B5 年度报告——功能清单全部收官（A 5/5、B 5/5，A2 待平台配置激活）
