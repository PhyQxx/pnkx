...
export default {
    ...
    watch: {
        modelValue(newVal) {
            if (this.cherryInstance && newVal !== this.content) {
                this.content = newVal
                this.setMarkdown(newVal || '', true)
            }
        }
    },
    mounted() {
    ...
    methods: {
        ...
        initCherryMD(value, config) {
            ...
            const defaultValue = value || this.modelValue
            /**
             * 自定义一个自定义菜单
            ...
            const customMenuC = Cherry.createMenuHook('帮助中心', {
                ...
                subMenuConfig: [{
                    ...
                    name: 'markdown教程',
                    onclick: (event) => {
                        this.cherryInstance.toolbar.menus.hooks.customMenuCName.fire(null, 'markdown')
                    }
                },
                    {
                        noIcon: true,
                        name: 'Emoji 表情',
                        onclick: (event) => {
                            this.cherryInstance.toolbar.menus.hooks.customMenuCName.fire(null, 'Emoji')
                        }
                    },
                    {
                        noIcon: true,
                        name: '公式编辑器',
                        onclick: (event) => {
                            this.cherryInstance.toolbar.menus.hooks.customMenuCName.fire(null, 'formula')
                        }
                    },
                    {
                        noIcon: true,
                        name: '完整示例',
                        onclick: (event) => {
                            this.cherryInstance.toolbar.menus.hooks.customMenuCName.fire(null, 'Example')
                        }
                    },
                ]
            });
        ...
        fileUpload(file) {
            ...
            formData.append('fileType', 'bksc');
            const request = new XMLHttpRequest()
            // 图片上传路径修改为自己连接
            request.open('POST', import.meta.env.VUE_APP_BASE_API + '/common/upload')
        ...
        onloadCallback(oEvent) {
            ...
            if (resp.code === 200) {
                let fileSuffix = resp.fileName.slice(resp.fileName.lastIndexOf('.'));
                if (/mp4|avi|rmvb/i.test(fileSuffix)) {
                    imgMdStr = `!video[${resp.fileName}](${resp.url})`;
                } else if (/mp3/i.test(fileSuffix)) {
                    imgMdStr = `!audio[${resp.fileName}](${resp.url})`;
                } else if (/bmp|gif|jpg|jpeg|png/i.test(fileSuffix)) {
        ...
        setMarkdown(content, keepCursor = false) {
            if (!this.cherryInstance) { // 未加载则重新初始化
                this.initCherryMD(content)
                return
            }
            this.cherryInstance.setMarkdown(content, keepCursor)
        },
        getCherryContent() {
             // 获取markdown内容
            return this.cherryInstance.getMarkdown()
        },
        getCherryHtml() {
            return this.cherryInstance.getHtml()
        },
        getData() {
            return this.cherryInstance.getHtml()
        },
         ...
         */
        exportMD(type = 'pdf') {
            this.cherryInstance.export(type)
        },
        /**
        ...
        switchModel(model) {
            if (this.isInit()) {
                this.cherryInstance.switchModel(model)
            }
        },
        insert(content, isSelect = false, anchor = [], focus = true) {
            this.cherryInstance.insert(content, isSelect, anchor, focus)
        },
        isInit() {
