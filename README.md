...
- **记账系统**：收支记录、分类管理、账户管理、消费统计、月度账单导出
- **记账预算**：月度总预算 / 分类预算、进度条、超支实时提醒
- **周期记账**：房租等固定支出按月/周规则自动生成记录
- **AI 记账**：一句话记账解析、批量文本导入
- **日记功能**：日记编写、心情记录、日记分析
- **待办事项**：任务管理、智能提醒
- **纪念日提醒**：情侣纪念日倒计时
- **家庭日历**：周视图、日程管理
...
- **读书管理**：个人书库、阅读进度
- **AI 生活报告**：周报 / 月报 / 年度报告（含月度支出分布、分类 Top、年度关键词）

### 系统管理（后台）

- 用户、角色、菜单、字典管理
- 文件管理：分片上传、进度显示
- 定时任务（Quartz）：数据库自动备份（FTP 归档保留 30 天）、周期记账等
- App 离线推送（uniPush 2.0，配置即用）
- 代码生成器
- 系统监控
